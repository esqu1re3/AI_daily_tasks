...
class BotService:
    ...
    def _activate_user_with_token(self, activation_token, telegram_user, db, bot, message):
        ...
        try:
            ...
            # === НОВОЕ: если пользователь — админ этой группы, просим использовать /admin ===
            if telegram_user.username and telegram_user.username.lower() == (target_group.admin_username or '').lower():
                bot.reply_to(
                    message,
                    f"Вы назначены администратором группы '{target_group.name}'.\n\nДля активации используйте команду /admin, а не /start."
                )
                return
