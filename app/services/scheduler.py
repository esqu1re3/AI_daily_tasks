...
def send_morning_questions():
    ...
    try:
        ...
        # Получаем активных И верифицированных пользователей
        active_users = db.query(User).filter(
            User.is_active == True,
            User.is_verified == True  # Добавляем проверку верификации
        ).all()
        
        if not active_users:
            logger.info("Нет активных верифицированных пользователей для рассылки")
            return
...
def generate_summary_after_timeout():
    ...
    db = SessionLocal()
    try:
        # Получаем активных верифицированных пользователей
        active_users = db.query(User).filter(
            User.is_active == True,
            User.is_verified == True  # Добавляем проверку верификации
        ).all()
        ...
        if not active_users:
            logger.info("Нет активных верифицированных пользователей для генерации сводки")
            return
...
def process_user_response(user, response_text):
    ...
    try:
        ...
        if db_user:
            ...
            # Проверяем, ответили ли все активные верифицированные пользователи (досрочная отправка)
            active_users = db.query(User).filter(
                User.is_active == True,
                User.is_verified == True  # Добавляем проверку верификации
            ).all()
...
def start_scheduler():
    ...
    try:
        ...
        if not scheduler.running:
            scheduler.start()
            logger.info("✅ Scheduler запущен (утренняя рассылка в 9:00 Asia/Bishkek, только верифицированным пользователям)")
